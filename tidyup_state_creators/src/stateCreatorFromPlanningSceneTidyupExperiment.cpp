#include "tidyup_state_creators/stateCreatorFromPlanningSceneTidyupExperiment.h"
...
PLUGINLIB_EXPORT_CLASS(tidyup_state_creators::StateCreatorFromPlanningSceneTidyupExperiment, continual_planning_executive::StateCreator)

namespace tidyup_state_creators
{
    StateCreatorFromPlanningSceneTidyupExperiment::StateCreatorFromPlanningSceneTidyupExperiment()
    {
    	ros::NodeHandle nh;
    ...
    void StateCreatorFromPlanningSceneTidyupExperiment::initialize(const std::deque<std::string>& arguments)
    {
    }

    bool StateCreatorFromPlanningSceneTidyupExperiment::fillState(SymbolicState& state)
    {
    	...
    	ROS_DEBUG_STREAM("StateCreatorFromPlanningSceneTidyupExperiment::" << __func__ << ": number of collision objects in planning scene: "
		...
		{
    		ROS_DEBUG_STREAM("StateCreatorFromPlanningSceneTidyupExperiment::" << __func__ << ": processing object: " << object.id);
            ...
            if (StringUtil::startsWith(object.id, "coke"))
            	addObjectToSymbolicState(state, object, "can");
            else if (StringUtil::startsWith(object.id, "cube"))
            	addObjectToSymbolicState(state, object, "cube");
//    		addObjectToSymbolicState(state, object, "movable_object");
            ...
            {
                ROS_INFO_STREAM("StateCreatorFromPlanningSceneTidyupExperiment::" << __func__ << ": putting " << object.id << " on " << closest_table);
                Predicate pOn;
                pOn.name = "object-on";
            	...
            	ROS_WARN("StateCreatorFromPlanningSceneTidyupExperiment::%s: NO matching Table found for object: %s",
            ...
            {
            	ROS_WARN("StateCreatorFromPlanningSceneTidyupExperiment::%s: setting predicate %s %s %s to false", __func__,
...
//    		addObjectToSymbolicState(state, object, "movable_object");
    	...
    	{
    		ROS_INFO("StateCreatorFromPlanningSceneTidyupExperiment::%s: removing object %s from symbolic state", __func__, (*it).c_str());
    ...
    void StateCreatorFromPlanningSceneTidyupExperiment::initializePlanningScene()
    {
        ROS_INFO("StateCreatorFromPlanningSceneTidyupExperiment::%s: Waiting for %s service.",
        ...
        {
            ROS_ERROR("StateCreatorFromPlanningSceneTidyupExperiment::%s: Failed to get initial planning scene.", __func__);
        }

        ROS_DEBUG("StateCreatorFromPlanningSceneTidyupExperiment::%s: Number of collision objects: %lu",
        		__func__, response.scene.world.collision_objects.size());

        setPlanningScene(response.scene);
    }

    void StateCreatorFromPlanningSceneTidyupExperiment::setPlanningScene(const moveit_msgs::PlanningScene& scene)
    {
    	planningScene_ = scene;
    }

    void StateCreatorFromPlanningSceneTidyupExperiment::initializeTables(const SymbolicState& currentState)
    {
...
//            }
    ...
    bool StateCreatorFromPlanningSceneTidyupExperiment::extractPoseStampedFromCollisionObject(const moveit_msgs::CollisionObject& co,
    ...
    void StateCreatorFromPlanningSceneTidyupExperiment::addObjectToSymbolicState(SymbolicState& state, const moveit_msgs::CollisionObject& co,
    		const std::string& objectType)
    {
        ...
        ROS_DEBUG_STREAM("StateCreatorFromPlanningSceneTidyupExperiment::" << __func__ << " object: " << co.id
        ...
        {
        	ROS_ERROR("StateCreatorFromPlanningSceneTidyupExperiment::%s: object:%s does not have a pose!", __func__, co.id.c_str());
        	return;
        }
    ...
    bool StateCreatorFromPlanningSceneTidyupExperiment::doesObjectTypeExist(const string& objectType)
    {
    	std::string types[] = { "pose", "frameid", "location", "table", "movable_object",
    			"arm", "arm_state", "can", "cube" };
    	std::set<std::string> objectTypes(types, types + sizeof(types) / sizeof(types[0]));
    	ROS_ASSERT(objectTypes.size() == 9);
    	...
    	{
    		ROS_ERROR("StateCreatorFromPlanningSceneTidyupExperiment::%s: Object Type %s does not exist "
    				"- maybe typo", __func__, objectType.c_str());
    		return false;
    	}
    }

    std::string StateCreatorFromPlanningSceneTidyupExperiment::findMatchingTable(
    		...
    		const moveit_msgs::CollisionObject& co)
    {
        ...
        forEach(const moveit_msgs::CollisionObject& table, allCos)
        {
            ...
            {
            	ROS_DEBUG("StateCreatorFromPlanningSceneTidyupExperiment::%s: CO Name: %s was found in tables_ and object name is: %s",
                ...
                {
                	ROS_ERROR("StateCreatorFromPlanningSceneTidyupExperiment::%s: table:%s does not have a pose!", __func__, table.id.c_str());
                	continue;
                }
                ...
                {
                	ROS_WARN("StateCreatorFromPlanningSceneTidyupExperiment::%s: table pose in frame: %s and object pose in frame: %s",
                			__func__, table_pose_stamped.header.frame_id.c_str(), co.header.frame_id.c_str());
                	ROS_INFO("StateCreatorFromPlanningSceneTidyupExperiment::%s: converting object pose to frame %s", __func__,
                    ...
                    try {
                        ...
                        ROS_ERROR("StateCreatorFromPlanningSceneTidyupExperiment::%s: %s", __func__, ex.what());
                    }
                ...
                {
                	ROS_WARN("StateCreatorFromPlanningSceneTidyupExperiment::%s: collision object %s (h = %lf) is beneath table %s (h = %lf)"
                ...
                ROS_DEBUG("StateCreatorFromPlanningSceneTidyupExperiment::%s: Distance between %s and %s is %lf", __func__, table.id.c_str(), co.id.c_str(), distance);
