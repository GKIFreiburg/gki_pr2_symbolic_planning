#include "object_manipulation_actions/actionExecutorDetectExpectedObjects.h"
...
{
void ActionExecutorDetectExpectedObjects::initialize(const std::deque<std::string> & arguments)
{
    ActionExecutorService<tidyup_msgs::DetectExpectedGraspableObjects>::initialize(arguments);
    ...
    {
        serviceClientGraspability = _nh->serviceClient<tidyup_msgs::DetectExpectedGraspableObjects>(graspabilityServiceName);
...
bool ActionExecutorDetectExpectedObjects::fillGoal(tidyup_msgs::DetectExpectedGraspableObjects::Request & goal,
        const DurativeAction & a, const SymbolicState & current)
{
    ...
    // find all objects on that table
    Predicate pOn;
    pOn.name = "on";
    pOn.parameters.push_back("object");
    pOn.parameters.push_back(goal.static_object);
    ...
    for (SymbolicState::TypedObjectConstIterator objectsIterator = objectRange.first;
            objectsIterator != objectRange.second; objectsIterator++)
    {
        const string& object_name = objectsIterator->second;
        pOn.parameters[0] = object_name;
        bool onThisTable = false;
        current.hasBooleanPredicate(pOn, &onThisTable);
        if (onThisTable)
        {
            tidyup_msgs::GraspableObject graspable;
            graspable.name = object_name;
            Predicate pObjectInfo;
            pObjectInfo.name = "frame_id";
            pObjectInfo.parameters.push_back(object_name);
            current.hasObjectFluent(pObjectInfo, &graspable.pose.header.frame_id);
            pObjectInfo.name = "x";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.position.x);
            pObjectInfo.name = "y";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.position.y);
            pObjectInfo.name = "z";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.position.z);
            pObjectInfo.name = "qx";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.orientation.x);
            pObjectInfo.name = "qy";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.orientation.y);
            pObjectInfo.name = "qz";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.orientation.z);
            pObjectInfo.name = "qw";
            current.hasNumericalFluent(pObjectInfo, &graspable.pose.pose.orientation.w);
            pObjectInfo.name = "graspable-from";
            pObjectInfo.parameters.push_back(a.parameters[0]);
            pObjectInfo.parameters.push_back("right_arm");
            bool grasp = false;
            current.hasBooleanPredicate(pObjectInfo, &grasp);
            graspable.reachable_right_arm = grasp;
            pObjectInfo.parameters.push_back("left_arm");
            current.hasBooleanPredicate(pObjectInfo, &grasp);
            graspable.reachable_left_arm = grasp;
        }
    }
    return true;
}

void ActionExecutorDetectExpectedObjects::updateState(bool& success,
        tidyup_msgs::DetectExpectedGraspableObjects::Response & response,
        const DurativeAction & a, SymbolicState & current)
{
    ...
    if (success)
    {
        ...
        for (std::vector<tidyup_msgs::WipeGoal>::iterator it = response.wipe_goals.begin(); it != response.wipe_goals.end(); it++)
        {
            ...
            bool wiped;
            if (!current.hasBooleanPredicate(wipedPred, &wiped))
            {   // only set if unknown
        ...
        pair<SymbolicState::TypedObjectConstIterator, SymbolicState::TypedObjectConstIterator> wipePointRange = current.getTypedObjects().equal_range("wipe_point");
        std::set<std::string> nonMatchedWipePoints;
        for (SymbolicState::TypedObjectConstIterator objectIterator = wipePointRange.first; objectIterator != wipePointRange.second; objectIterator++)
        {
...
std::string ActionExecutorDetectExpectedObjects::findStaticObjectForLocation(const std::string& location, const SymbolicState & current) const
{
    Predicate p;
