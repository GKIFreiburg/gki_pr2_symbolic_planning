#include "object_manipulation_actions/actionExecutorInspectTable.h"
...
PLUGINLIB_EXPORT_CLASS(object_manipulation_actions::ActionExecutorInspectTable, continual_planning_executive::ActionExecutorInterface)

namespace object_manipulation_actions
{

void ActionExecutorInspectTable::initialize(const std::deque<std::string> & arguments)
{
...
bool ActionExecutorInspectTable::canExecute(const DurativeAction & a, const SymbolicState & currentState) const
{
	return a.name == action_name_;
}

bool ActionExecutorInspectTable::executeBlocking(const DurativeAction & a, SymbolicState & currentState)
{
	...
	{
		ROS_ERROR("ActionExecutorInspectTable::%s: Could not extract pose of table:%s from symbolic state", __func__, surface_name.c_str());
		return false;
	}
...
void ActionExecutorInspectTable::cancelAction()
{
}

bool ActionExecutorInspectTable::executePointHead(const geometry_msgs::PoseStamped tablePose)
{
...
bool ActionExecutorInspectTable::executeUpdatePlanningSceneFromORK(bool verify_planning_scene_update,
		...
		bool merge_tables)
{
	...
	if (finished_before_timeout)
	{
		...
		ROS_DEBUG("ActionExecutorInspectTable::%s: ORK to Planning Scene Action finished: %s", __func__, state.toString().c_str());
		if (state == actionlib::SimpleClientGoalState::SUCCEEDED)
		{
...
bool ActionExecutorInspectTable::executeTurnHead(const int degrees)
{
	...
	if (!head_group_->setJointValueTarget( jointValues ))
		ROS_ERROR("ActionExecutorInspectTable::%s: joint values out of bounds!", __func__);
