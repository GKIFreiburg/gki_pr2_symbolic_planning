#include "object_manipulation_actions/InspectTable.h"
...
PLUGINLIB_EXPORT_CLASS(object_manipulation_actions::InspectTable, continual_planning_executive::ActionExecutorInterface)

namespace object_manipulation_actions
{

void InspectTable::initialize(const std::deque<std::string> & arguments)
{
  ...
  actionOrkToPs_ = new actionlib::SimpleActionClient<ork_to_planning_scene_msgs::UpdatePlanningSceneFromOrkAction>(
      action_topic_ork_ps_, true);
  ROS_INFO("ActionExecutorInspectTable::%s: Waiting for ork_to_planning_scene "
  ...
  ROS_INFO("ActionExecutorInspectTable::%s: Waiting for head_traj_controller/point_head_action "
      "action server to start.", __func__);
  actionPointHead_->waitForServer();

  ROS_INFO("ActionExecutorInspectTable::%s: Action clients are ready", __func__);

  ros::NodeHandle nhPriv("~");
  // Namespace is "/continual_planning_executive"(/wave_head_degrees)
  nhPriv.param("wave_head_degrees", degrees_, 30);
...
bool InspectTable::canExecute(const DurativeAction & a, const SymbolicState & currentState) const
{
  return a.name == action_name_;
}

bool InspectTable::executeBlocking(const DurativeAction & a, SymbolicState & currentState)
{
  ...
  {
    ROS_ERROR("ActionExecutorInspectTable::%s: Could not extract pose of table:%s from symbolic state", __func__,
        surface_name.c_str());
    return false;
  }
  ...
  if (! executeUpdatePlanningSceneFromORK(verify_planning_scene_update, expected_objects, add_tables, table_prefix,
      merge_tables))
    return false;
...
void InspectTable::cancelAction()
{
}

bool InspectTable::executePointHead(const geometry_msgs::PoseStamped tablePose)
{
  ROS_INFO("ActionExecutorInspectTable::%s: Sending PointHead request.", __func__);
  ...
  if (finished_before_timeout)
  {
    ...
    ROS_DEBUG("ActionExecutorInspectTable::%s: Point Head Action finished: %s", __func__, state.toString().c_str());
    if (state != actionlib::SimpleClientGoalState::SUCCEEDED)
    {
      ROS_ERROR("ActionExecutorInspectTable::%s: Point Head Action failed.", __func__);
      //return false;
    }
  }
...
bool InspectTable::executeUpdatePlanningSceneFromORK(bool verify_planning_scene_update,
    const std::vector<std::string>& expected_objects, bool add_tables, std::string table_prefix, bool merge_tables)
{
  // execute action ork-to-planning-scene
  ROS_INFO("ActionExecutorInspectTable::%s: Sending UpdatePlanningSceneFromORK request.", __func__);
  ...
  ROS_INFO("ActionExecutorInspectTable::%s: number of expected_objects: %lu", __func__, expected_objects.size());
  ...
  if (finished_before_timeout)
  {
    ...
    ROS_DEBUG("ActionExecutorInspectTable::%s: ORK to Planning Scene Action finished: %s", __func__,
        state.toString().c_str());
    if (state == actionlib::SimpleClientGoalState::SUCCEEDED)
    {
      ROS_INFO("ActionExecutorInspectTable::%s: ORK to Planning Scene Action succeeded.", __func__);
      ...
      for (size_t i = 0; i < seen_objects.size(); i++)
      {
        if (expected_objects_.erase(seen_objects.at(i)) > 0)
          ROS_INFO("ActionExecutorInspectTable::%s: Object %s has been detected, no longer an expected object",
    ...
    {
      ROS_ERROR("ActionExecutorInspectTable::%s: ORK to Planning Scene Action failed.", __func__);
      return false;
    }
  }
  ROS_ERROR("ActionExecutorInspectTable::%s: ORK to Planning Scene Action timed out.", __func__);
  return false;
}

bool InspectTable::executeTurnHead(const int degrees)
{
  ROS_INFO("ActionExecutorInspectTable::%s: turn head by %d degrees.", __func__, degrees);
  ...
  for (size_t i = 0; i < current_joint_values.size(); i++)
  {
    ...
    ROS_DEBUG("ActionExecutorInspectTable::%s: %s - %lf - old value: %lf", __func__, jointValue.first.c_str(),
  ...
  {
    ROS_WARN_STREAM(action_name_<<": joint values out of bounds");
  }
  return symbolic_planning_utils::MoveGroupInterface::getInstance()->planExecuteVerify(head_group_, action_name_)
      == moveit::planning_interface::MoveItErrorCode::SUCCESS;
}

void InspectTable::renameTableCollisionObject(const std::string& tableName)
{
  ...
  {
    ROS_INFO("ActionExecutorInspectTable::%s: Found object with id: %s, renaming to: %s", __func__, co.id.c_str(), tableName.c_str());
    tableCO = co;
