#include "object_manipulation_actions/actionExecutorPutdownObjectGrounding.h"
...
//        object_manipulation_actions::ActionExecutorPutdownObjectGrounding,
//        continual_planning_executive::ActionExecutorInterface)
PLUGINLIB_EXPORT_CLASS(object_manipulation_actions::ActionExecutorPutdownObjectGrounding, continual_planning_executive::ActionExecutorInterface)

namespace object_manipulation_actions
{

ActionExecutorPutdownObjectGrounding::ActionExecutorPutdownObjectGrounding()
{
	...
	ROS_INFO("ActionExecutorPutdownObjectGrounding::%s: Action executor is ready", __func__);
}
...
bool ActionExecutorPutdownObjectGrounding::executeBlocking(const DurativeAction & a, SymbolicState & currentState)
{
	...
	{
		ROS_ERROR("ActionExecutorPutdownObjectGrounding::%s: Could not find surface_object %s", __func__, table.c_str());
		return false;
	}
	...
	{
		ROS_ERROR("ActionExecutorPutdownObjectGrounding::%s: Could not find attached_object %s", __func__, movable_obj.c_str());
		return false;
	}
...
//    ROS_WARN_STREAM("z_above_table: " << z_above_table_);
    ...
    if (locs.size() == 0)
    {
    	ROS_ERROR("ActionExecutorPutdownObjectGrounding::%s: Could not determine any place location.", __func__);
    	return false;
    }
...
void ActionExecutorPutdownObjectGrounding::cancelAction()
{
}
